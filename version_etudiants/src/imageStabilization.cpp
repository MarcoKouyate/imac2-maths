...
int main(int argc, char **argv)
{
  ...
  while(loop){ // la boucle du temps
    ...
    // Etape 2 : Centering (matrice de translation pour centrer l'image)
    ...
    x(0) = cos(std::max(-angleMax, std::min(acos(x(0)),angleMax)));
    x(1) = sin(std::max(-angleMax, std::min(asin(x(1)),angleMax)));

    // Etape 6 : extract rotation
    Eigen::Matrix3d M = Eigen::Matrix3d::Identity();
    M(0,0) = M(1,1) = x(0); //cos
    M(0,1) = x(1); //sin
    M(1,0) = -x(1); //-sin

    // Etape 8 : Enforce rotation
    ...
    //Etape 6 : extract translation
    M(0,2) = x(2);
    M(1,2) = x(3);

    //Etape 9 : clamp translation
    double translationMax = 100;
    M(0,2) = std::max(-translationMax, std::min(M(0,2), translationMax));
    M(1,2) = std::max(-translationMax, std::min(M(1,2), translationMax));
